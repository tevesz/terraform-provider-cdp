...
func (r *dwClusterResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dw_azure_cluster"
}
...
func (r *dwClusterResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	clusterParams := operations.NewCreateAzureClusterParamsWithContext(ctx).
		WithInput(plan.convertToCreateAzureClusterRequest())

	// Create new Azure cluster
	response, err := r.client.Dw.Operations.CreateAzureCluster(clusterParams)
	if err != nil {
		resp.Diagnostics.AddError(
			"Error creating Data Warehouse Azure cluster",
	...
	if !(plan.PollingOptions != nil && plan.PollingOptions.Async.ValueBool()) {
		...
		stateConf := &retry.StateChangeConf{
			...
			Timeout:      utils.GetPollingTimeout(&plan, 40*time.Minute),
			PollInterval: 30 * time.Second,
			Refresh:      r.stateRefresh(ctx, clusterID, &callFailedCount, utils.GetCallFailureThreshold(&plan, 3)),
		}
		if _, err = stateConf.WaitForStateContext(ctx); err != nil {
			resp.Diagnostics.AddError(
				"Error waiting for Data Warehouse Azure cluster",
				"Could not create cluster, unexpected error: "+err.Error(),
			)
			return
		}
...
func (r *dwClusterResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if _, err := r.client.Dw.Operations.DeleteCluster(op); err != nil {
		resp.Diagnostics.AddError(
			"Error deleting Data Warehouse Azure cluster",
	...
	if !(state.PollingOptions != nil && state.PollingOptions.Async.ValueBool()) {
		...
		stateConf := &retry.StateChangeConf{
			...
			Delay:        30 * time.Second,
			Timeout:      utils.GetPollingTimeout(&state, 40*time.Minute),
			PollInterval: 30 * time.Second,
			Refresh:      r.stateRefresh(ctx, clusterID, &callFailedCount, utils.GetCallFailureThreshold(&state, 3)),
		}
		if _, err := stateConf.WaitForStateContext(ctx); err != nil {
			resp.Diagnostics.AddError(
				"Error waiting for Data Warehouse Azure cluster",
		...
		if err != nil {
			...
			*callFailedCount++
			if *callFailedCount <= callFailureThreshold {
				tflog.Warn(ctx, fmt.Sprintf("could not describe Data Warehouse Azure cluster "+
					"due to [%s] but threshold limit is not reached yet (%d out of %d).", err.Error(), callFailedCount, callFailureThreshold))
				return nil, "", nil
			}
			tflog.Error(ctx, fmt.Sprintf("error describing Data Warehouse Azure cluster due to [%s] "+
